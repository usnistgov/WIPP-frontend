pipeline {
  ...
  triggers {
    ...
    stages {
      ...
      stage('Build npm & Pushing Artifactory') {
		steps {
                   script {   
	    ...
	    withCredentials([string(credentialsId: 'ARTIFACTORY_USER', variable: 'ARTIFACTORY_USER'),
                   ...
                   sh "curl -u${ARTIFACTORY_USER}:${ARTIFACTORY_TOKEN} -T ${BUILD_VERSION}.tar.gz https://builds.aws.labshare.org/artifactory/labshare/WIPP-frontend/${BUILD_VERSION}.tar.gz"
                }
	          } 
	    }
        stage('Docker build & push image to AWS ECR') {
            steps {
              withCredentials([string(credentialsId: 'ARTIFACTORY_USER', variable: 'ARTIFACTORY_USER'),
                               string(credentialsId: 'ARTIFACTORY_TOKEN', variable: 'ARTIFACTORY_TOKEN')]) {
                script {
                        docker.build("wipp_frontend", "--build-arg SOURCE_FOLDER=. --build-arg BUILD_VERSION=${BUILD_VERSION} --build-arg ARTIFACTORY_USER=${ARTIFACTORY_USER} --build-arg ARTIFACTORY_TOKEN=${ARTIFACTORY_TOKEN} --no-cache ./")
                  docker.withRegistry('https://684150170045.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:aws-jenkins-build') {
                      docker.image("wipp_frontend").push("current")
                          }
		                  }
	                }
              }
